...
describe Admin::PurchasesController, :vcr do
  ...
  describe "#show" do
    ...
    it "finds the purchase correctly when loaded via external id" do
      ...
      expect(response.body).to include("Admin/Purchases/Show")
  ...
  describe "POST refund_for_fraud" do
    let(:purchase) { create(:purchase_in_progress, chargeable: create(:chargeable), purchaser: create(:user)) }
  ...
  describe "POST undelete" do
    ...
    let(:is_deleted_by_buyer) { true }
    let!(:purchase) { create(:purchase, purchaser:, is_deleted_by_buyer:) }
    let(:comment_content) { "Purchase undeleted by Admin (#{admin_user.email})" }
    ...
    context "when purchase is already undeleted" do
      let(:is_deleted_by_buyer) { false }

      it "does not change the purchase" do
        expect do
          post :undelete, params: { id: purchase.id }
    ...
    context "when purchase without purchaser" do
      let(:purchaser) { nil }

      it "undeletes the purchase" do
        expect do
          post :undelete, params: { id: purchase.id }
