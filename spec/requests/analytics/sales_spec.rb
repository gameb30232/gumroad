...
describe "Sales analytics", :js, :sidekiq_inline, :elasticsearch_wait_for_refresh, type: :system do
  ...
  context "with views and sales" do
    ...
    it "shows the sales chart" do
      visit sales_dashboard_path(from: "2023-12-01", to: "2023-12-31")
      expect(page).to have_css("section svg circle", count: 31) # Chart data points
      expect(page).to have_css("section svg path.recharts-rectangle", count: 5) # Chart bars

      chart = find("section svg").find(:xpath, "./..")
      chart.hover
      expect(chart).to have_tooltip(text: "3 views\n1 sale\n(33.3% conversion)\n$5\nSaturday, December 16")

      select "Monthly", from: "Aggregate by"
      expect(page).to have_css("section svg circle", count: 1) # Chart data points
      expect(page).to have_css("section svg path.recharts-rectangle", count: 2) # Chart bars
      chart.hover
      ...
      expect(page).to have_css("section svg path.recharts-rectangle", count: 3) # Chart bars
      ...
      expect(page).to have_css("section svg circle", count: 2) # Chart data points
      expect(page).to have_css("section svg path.recharts-rectangle", count: 2) # Chart bars
    end
