...
describe "Sales analytics", :js, :sidekiq_inline, :elasticsearch_wait_for_refresh, type: :system do
  ...
  context "with views and sales" do
    ...
    it "shows the sales chart" do
      ...
      expect(page).to have_css('[aria-label="chart-bar"]', count: 5)
      ...
      expect(page).to have_css('[aria-label="chart-bar"]', count: 2)
      chart.hover
      ...
      expect(page).to have_css('[aria-label="chart-bar"]', count: 3)
